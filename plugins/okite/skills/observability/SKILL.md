...
GitHub 検索構文は相対日付をサポートしないため、実行時に 30 日前の日付を計算して埋め込む。

```
mcp__github__search_pull_requests with query "author:@me created:><YYYY-MM-DD>"
# <YYYY-MM-DD> は実行時に 30 日前を埋める。例:
#   date -u -d '30 days ago' +%Y-%m-%d   (GNU date)
#   date -u -v-30d +%Y-%m-%d             (BSD/macOS date)
```
